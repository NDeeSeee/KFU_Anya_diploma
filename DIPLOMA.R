...
FMI_samples <- fread("FMI samples ID's.csv") %>%
  as_tibble() %>%
  filter(FMI_samples, msi == "MT-L")
...
frame <- fread("FMI variants.csv") %>%
  ...
  mutate(
    ...
    DB_MV_consensus = case_when(
      DB_status == "LoF" ~ "LoF",
      DB_status == "WT" ~ "WT",
      is.na(DB_status) &
        (VAMP_status == "LoF" | MAVE_status == "LoF") ~ "LoF",
      is.na(DB_status) &
        (VAMP_status == "WT" & MAVE_status == "WT") ~ "WT"
    ),
    ultimate_consensus = case_when(
      DB_MV_consensus == "LoF" ~ "LoF",
      DB_MV_consensus == "WT" ~ "WT",
      gene == "PTEN" &
        (variantClass_v2 == "truncation" |
           codingType_SV == "splice") ~ "LoF"
    )
  )
...
# PTEN column names
pten_types <-
  names(frame_grouped)[which(str_detect(names(frame_grouped), "PTEN"))]

# Other genes
gene_names <-
  names(frame_grouped)[which(!str_detect(names(frame_grouped), "PTEN") &
                               str_detect(names(frame_grouped), "presence"))]

# Start
gene_name_vector <-
  pten_type_vector <-
  odds_ratio_vector <-
  upper_conf_vector <- lower_conf_vector <- p_value_vector <- c()

for (gene in gene_names) {
  for (pten_type in pten_types) {
    if (all(dim(table(
      select(frame_grouped, pten_type, gene)
    )) == 2)) {
      f_test_results <-
        fisher.test(table(select(frame_grouped, pten_type, gene)))
      gene_name_vector <- c(gene_name_vector, gene)
      pten_type_vector <- c(pten_type_vector, pten_type)
      odds_ratio_vector <-
        c(odds_ratio_vector, f_test_results$estimate)
      upper_conf_vector <-
        c(upper_conf_vector, f_test_results$conf.int[2])
      lower_conf_vector <-
        c(lower_conf_vector, f_test_results$conf.int[1])
